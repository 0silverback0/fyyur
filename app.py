...
def venues():
  ...
  # }]
    ...
    for v in venues:
      locs.add( (v.city, v.state) )
      
    already_city = []

    for loc in locs:
      if loc not in already_city:
        already_city.append(loc)
      
    data = []

    for city_state in already_city:
      city = city_state[0]
      state = city_state[1]
...
def show_venue(venue_id):
  ...
  shows = venue.shows
  artist_ids = [show.artist_id for show in venue.shows]
  now = datetime.now()
  count = 0
  ...
  for id in artist_ids:
      ...
      if shows[count].start_time > str(now):
        upcoming_shows.append({
            "artist_id": artist.id,
            "artist_name": artist.name,
            "artist_image_link": artist.image_link,
            "start_time": shows[count].start_time 
        })
  ...
  venue.past_shows = [show for show in venue.shows if show.start_time < str(now)] 
...
def show_artist(artist_id):
  ...
  data.past_shows = [show for show in shows if show.start_time < str(now)]
  data.past_shows_count = len(data.past_shows)
  upcoming_shows = []

  for show in shows:
    if show.start_time > str(now):
      upcoming_shows.append( {
